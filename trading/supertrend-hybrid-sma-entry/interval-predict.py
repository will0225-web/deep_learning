...
# 定義 model 名稱與檔案位置
v1_model_name = 'supertrend_hybrid_sma_entry'
...
def execute_trade(predicted_result, confidence, data, model_type=None, stop_loss=None, take_profit=None, 
    ...
    if predicted_result in predicted_long_results or predicted_result in predicted_short_results:
        ...
        volume_sma = data.iloc[-1]['Volume_SMA']
        meet_entry_conditions = check_entry_conditions(predicted_result, confidence, volume, supertrend, kline_color, volume_sma, 25000)
...
def set_v2_tp(v2_prediction, v2_confidence):
    ...
    if v2_prediction == 1 and not(0.36 <= v2_confidence <= 0.375):
        if 0.34 <= v2_confidence <= 0.44:
            take_profit = 0.015
        elif 0.23 <= v2_confidence <= 0.26:
            take_profit = 0.03
    elif v2_prediction == 2 and not(0.35 <= v2_confidence >= 0.36):
        if 0.32 <= v2_confidence <= 0.37:
            take_profit = 0.01
        elif 0.21 <= v2_confidence <= 0.24:
            take_profit = 0.01
    elif v2_prediction == 3 and not(0.258 <= v2_confidence <= 0.264 or 0.27 <= v2_confidence <= 0.278):
        if 0.252 <= v2_confidence <= 0.28:
            take_profit = 0.015
    elif v2_prediction == 4 and not(0.255 <= v2_confidence <= 0.263): # 需調整，容易輸
        if 0.245 <= v2_confidence <= 0.25:
            ...
            take_profit = 0.015
    elif v2_prediction == 6 and not(0.246 <= v2_confidence <= 0.252 or 0.254 <= v2_confidence <= 0.259):
        if 0.243 <= v2_confidence <= 0.265:
            take_profit = 0.015
    elif v2_prediction == 7 and not(0.47 <= v2_confidence <= 0.475 or 0.39 <= v2_confidence <= 0.44):
        if 0.46 <= v2_confidence <= 0.49:
            take_profit = 0.025 # 或0.03
        elif 0.37 <= v2_confidence <= 0.49:
            take_profit = 0.015 # 或0.01(0.01比較保險)
    elif v2_prediction == 8 and not(0.44 <= v2_confidence <= 0.47):
        if 0.4 <= v2_confidence <= 0.53:
            take_profit = 0.02
    elif v2_prediction == 9:
        if 0.2 <= v2_confidence <= 0.27:
            take_profit = 0.02
    elif v2_prediction == 10 and not(0.246 <= v2_confidence <= 0.25):
        if 0.243 <= v2_confidence <= 0.255:
            take_profit = 0.01

    return take_profit

def check_entry_conditions(predicted_result, confidence, volume=None, supertrend=None, kline_color=None, volume_sma=None, volume_sma_excess=None):
    if supertrend == 1:
        if predicted_result == 0 and entry_confidence_0[0] <= confidence <= entry_confidence_0[1] and volume >= volume_sma + volume_sma_excess:
            return True
        elif predicted_result == 2 and entry_confidence_2[0] <= confidence <= entry_confidence_2[1] and volume >= volume_sma + volume_sma_excess:
            return True
        elif predicted_result == 4 and entry_confidence_4[0] <= confidence <= entry_confidence_4[1] and volume >= volume_sma + volume_sma_excess:
            return True
        else:
            return False
    elif supertrend == -1:
        if predicted_result == 1 and entry_confidence_1[0] <= confidence <= entry_confidence_1[1] and volume >= volume_sma + volume_sma_excess:
            return True
        elif predicted_result == 3 and entry_confidence_3[0] <= confidence <= entry_confidence_3[1] and volume >= volume_sma + volume_sma_excess:
            return True
        elif predicted_result == 5 and entry_confidence_5[0] <= confidence <= entry_confidence_5[1] and volume >= volume_sma + volume_sma_excess:
            return True
        else:
            return False
    else:
...
if time.time() % 900 < 50:
    if position == 0:
        ...
        data_2 = fetch_data_hybrid(symbol, interval, v2_data_file_name, fetch_data_length, 'supertrend', calculated_data)
