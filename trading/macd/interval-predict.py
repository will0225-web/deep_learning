...
model_path = 'trained_models/1707990480_softmax_loss-0.9399_accuracy-0.6760_f1-0.2817_best'
data_file_name = 'ETHUSDT_15m_realtime_caculated_480.csv'
...
# 策略參數設定
# 模型預測結果之條件參數
predicted_long_results = [2]
predicted_short_results = [1]
...
def execute_trade(predicted_result, confidence, data, model_type=None, stop_loss=None, take_profit=None, trailing_threshold=None, trailing_return_threshold=None):
    ...
    if predicted_result in (predicted_long_results + predicted_short_results):
        ...
        if entry_position != '':
            ...
            send_order(symbol, f'{entry_position} with TP', model_type, sl_value=stop_loss * 100, sl_price=sl_price, tp_value=take_profit * 100, current_price=current_price)
...
def check_entry_conditions(predicted_result, confidence, volume=None, supertrend=None, kline_color=None):
    if predicted_result in (predicted_long_results + predicted_short_results) and confidence >= entry_confidence:
        return True
    else:
        return False
...
def close_position(data, position, position_info={}, model_type=None):
    ...
    if position == 1:
        ...
        if current_low <= ((1 - stop_loss) * entry_price):
            ...
            exit_condition = 'Long RSI Delta Weaken'
            execute_exit_trade(symbol, 'Exit Long', take_profit=take_profit, stop_loss=stop_loss, close_price=current_price, exit_condition=exit_condition, model_type=model_type)
    ...
    # Short position exit conditions
        ...
        if current_high >= ((1 + stop_loss) * entry_price):
            ...
            exit_condition = 'Short RSI Delta Weaken'
            execute_exit_trade(symbol, 'Exit Short', take_profit=take_profit, stop_loss=stop_loss, close_price=current_price, exit_condition=exit_condition, model_type=model_type)
        elif hold_count >= stable_hold_count_limit and max_profit < stable_price_percentage:
...
if time.time() % 900 < 50:
    if position == 0:
        ...
        save_predicted_result(predicted_result, confidence)
        execute_trade(predicted_result, confidence, data, model_type=model_name, stop_loss=model_sl, take_profit=model_tp)
    else:
