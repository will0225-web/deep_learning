...
def enhancement_strategy(X, predicted):
    ...
    trailing_return_threshold = 0.007

    take_profit_3_4 = 0.008
    stop_loss_3_4 = 0.012

    # current run best but not adjust
    ...
    entry_confidence_1 = (0.4, 1)
    entry_confidence_2 = (0.4, 1)
    entry_confidence_3 = (0.4, 0.7)
    entry_confidence_4 = (0.4, 0.7)
        ...
        if position != 0:
            if position == 1 and entry_predicted_target == 1:
                if (entry_price - original_low) / entry_price >= stop_loss:
                    ...
                    if max_profit >= take_profit or i - entry_index >= hold_count_limit or (max_profit >= trailing_threshold and current_low_profit <= max_profit - trailing_return_threshold):
                        # 結單
                        ...
                        elif i - entry_index >= hold_count_limit:
                ...
                if (original_high - entry_price) / entry_price >= stop_loss:
                    ...
                    if max_profit >= take_profit or i - entry_index >= hold_count_limit or (max_profit >= trailing_threshold and current_high_profit <= max_profit - trailing_return_threshold):
                        # 結單
                        if max_profit >= take_profit:
                            # 不超過take profit的percentage
            ...
            # if original_close > upper_band and predicted_target == 1 and entry_confidence_1[0] <= confidence <= entry_confidence_1[1]:
            #     position = 1
            ...
            # elif original_close < lower_band and predicted_target == 2 and entry_confidence_2[0] <= confidence <= entry_confidence_2[1]:
            #     position = -1
            ...
            if (predicted_target == 1 and confidence >= entry_confidence_1[0]) or (predicted_target == 3 and confidence >= entry_confidence_3[1]) or (predicted_target == 3 and confidence < entry_confidence_3[0]):
                position = 1
                ...
                entry_predicted_target = 1
            elif (predicted_target == 2 and confidence >= entry_confidence_2[0]) or (predicted_target == 4 and confidence >= entry_confidence_4[1]) or (predicted_target == 4 and confidence < entry_confidence_4[0]):
                position = -1
