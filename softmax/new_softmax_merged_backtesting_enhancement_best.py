...
def get_original_data(symbol, interval, cols):
    original_df = modules_data.get_binance_klines_backward(symbol, interval, "2023-11-30 23:59:59", 100000, cols, is_need_save_original_data=False, is_read_local=True, is_need_calculated=True)
    ...
    X = indicators.add_lower_low_higher_high(original_X, look_back=282)
    X = indicators.add_price_indicator(original_X, look_back=4320)

    original_X = original_X[(29904 + 1600):-300]
    original_X.reset_index(drop=True, inplace=True)
    original_y = original_y[(29904 + 1600):-300]
...
# Step 1: 獲取數據
df = modules_data.get_binance_klines_backward(symbol, interval, end_time_string, 20000, cols, is_need_save_original_data=False, is_read_local=False, is_need_calculated=True)
...
# 再拿掉前後無參考性資料
X = X[(price_look_back + 1600):]
X.reset_index(drop=True, inplace=True)
y = y[(price_look_back + 1600):]
...
# 选择要缩放的列
ochl_cols = ['Open', 'Close', 'High', 'Low', 'Hist', 'Middle Band', 'Upper Band', 'Lower Band']
ochl_data = X[ochl_cols]

robust_features = ['Open', 'Close', 'High', 'Low', 'Hist', 'Middle Band', 'Upper Band', 'Lower Band']
standard_features = []
minMax_features = []
...
current_cols = ['fibonacci_0.382', 'fibonacci_0.5', 'fibonacci_0.618', 'MACD_Cross', 'red_three_soldiers', 'green_three_soldiers', 'lower_low', 'higher_high', 'rsi_overbought', 'rsi_oversold', 'TR', 'ATR', 'Volume']
current_data = X[current_cols]

robust_features = ['fibonacci_0.382', 'fibonacci_0.5', 'fibonacci_0.618']
standard_features = ['TR', 'ATR', 'Volume']
minMax_features = []
...
# 使用look_back创建数据集
np_X, np_y = create_look_back_dataset(X_ochl_scaled, y_categorical, look_back)

# 載入已經訓練好的模型
# 獲取當前文件的絕對路徑
current_path = os.path.abspath(os.path.dirname(__file__))
parent_path = os.path.join(current_path, '..')
trained_model_name = f'{parent_path}/trained_models/1707990480_softmax_loss-0.9399_accuracy-0.6760_f1-0.2817_best'
model = tf.keras.models.load_model(trained_model_name, custom_objects={'multiclass_f1_score': multiclass_f1_score})

df = df[(price_look_back + 1600):]
...
# best的target目前是預測到前一個的target, 但是有一定準度, 所以優化如何下單
for i in range(0, len(predicted)):
    ...
    if position != 0:
        ...
        if predicted_target == 1 and confidence >= entry_confidence and continued_count >= min_continued_target_count:  # 预测为多头且当前无仓位
            ...
            # if current_profit < 0.009 and pre_close_low > -0.004:
            position = 1
            ...
            # if current_profit < 0.009 and pre_close_high > -0.004:
            position = -1
