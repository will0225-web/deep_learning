...
def set_y_multiple_label(df, max_lookahead=8, percentage=0.05):
...
#             if max_future_price >= current_close * (1 + percentage) and min_future_price <= current_close * (1 - percentage):
#                 y_label = 3  # 未来同时出现上涨和下跌5%的可能性
#             elif max_future_price >= current_close * (1 + percentage):
#                 y_label = 1  # 未来有上涨5%的可能性
#             elif min_future_price <= current_close * (1 - percentage):
#                 y_label = 2  # 未来有下跌5%的可能性
#             else:
#                 y_label = 0  # 未来没有明显的上涨或下跌
            
#             # 将新样本的特征和标签保存
#             new_sample = df.iloc[i].copy()
#             new_sample['lookahead'] = lookahead
#             new_sample['percentage'] = percentage
#             new_sample['y'] = y_label
...
def validation(test_data, model, preprocessor, cols, lookahead, percentage):
    # 预测并存储结果
    ...
    # input_data['up_percentage'] = percentage
    # input_data['down_percentage'] = -percentage
    ...
    # 过滤出预测概率大于0.9的行
    high_confidence_data = input_data[(probs.max(axis=1) > 0.9)]
    ...
    for class_label in range(probs.shape[1]):
        ...
        # 分子: predicted_class == y 且 prob >= 0.9 的样本数量
    ...
    input_data.to_csv('validation.csv', index=False)

    total_profit_percentage, trades = backtesting(input_data, lookahead, percentage)
    print(f"Total Profit Percentage: {total_profit_percentage}")
    return total_accuracy, class_accuracies, class_denominators, class_numerators
...
def backtesting(input_data, lookahead, percentage, fee=0.0012):
    ...
    for i in range(len(input_data) - lookahead):
        ...
        # 进场多头交易
        if predicted_class == 1 and prob_class_1 >= 0.9:
            ...
            if future_low <= sl_price:
                profit_percentage = -percentage - fee  # 止损损失加上手续费
            elif future_high >= tp_price:
                profit_percentage = percentage - fee  # 止盈利润减去手续费
            else:
                profit_percentage = 0  # 没有达到止盈或止损

            total_profit_percentage += profit_percentage
            trades.append(profit_percentage)

        # 进场空头交易
        elif predicted_class == 2 and prob_class_2 >= 0.9:
            future_high = input_data['High'].iloc[i+1:i+lookahead+1].max()  # 未来的最高价
            future_low = input_data['Low'].iloc[i+1:i+lookahead+1].min()  # 未来的最低价
            ...
            if future_high >= sl_price:
                profit_percentage = -percentage - fee  # 空头止损损失加上手续费
            elif future_low <= tp_price:
                profit_percentage = percentage - fee  # 空头止盈利润减去手续费
            else:
                profit_percentage = 0  # 没有达到止盈或止损
...
standard_features = ['Close', 'High', 'Low', 'Open', 'fibonacci_0.382', 'fibonacci_0.5', 'fibonacci_0.618', 'fibonacci_1', 'fibonacci_0', 'EMA_7', 'EMA_25', 'EMA_99', 'Upper Band', 'Lower Band', 'Middle Band', 'VWAP', 'up_percentage', 'down_percentage']
minMax_features = []
...
# model_big_trend = xgb.XGBClassifier(n_estimators=3000, max_depth=10, learning_rate=0.005, random_state=42, device='cuda', verbosity=1)
# model_big_trend = xgb.XGBClassifier(n_estimators=100, max_depth=5, learning_rate=0.5, random_state=42, device='cuda', verbosity=1)

model_big_trend.fit(X_scaled, y)
# model_big_trend.save_model('xgboost_big_trend.model')

total_accuracy, class_accuracies, class_denominators, class_numerators = validation(test_data, model_big_trend, preprocessor, cols, lookahead, percentage)


print(f"n_estimators: {n}, max_depth: {d}, learning_rate: {lr}, total_accuracy: {total_accuracy}, class_0_accuracy: {class_accuracies[0]}, class_1_accuracy: {class_accuracies[1]}, class_2_accuracy: {class_accuracies[2]}, class_3_accuracy: {class_accuracies[3]}, class_0_denominator: {class_denominators[0]}, class_1_denominator: {class_denominators[1]}, class_2_denominator: {class_denominators[2]}, class_3_denominator: {class_denominators[3]}, class_0_numerator: {class_numerators[0]}, class_1_numerator: {class_numerators[1]}, class_2_numerator: {class_numerators[2]}, class_3_numerator: {class_numerators[3]}")