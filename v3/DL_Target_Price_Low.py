...
def train_data(df, y):
    ...
    ochl_cols = ['Volume', 'Low', 'Close', 'High', 'Open', 'fibonacci_0.618', 'fibonacci_0.5', 'fibonacci_0.382', 'max_high_look_back_24h', 'min_low_look_back_24h', 'Support']

    X = df[ochl_cols]

    outliers = print_outliers_and_return(X)

    ochl_robust_features = ['Volume']
    ochl_standard_features = ['Low', 'Close', 'High', 'Open', 'fibonacci_0.618', 'fibonacci_0.5', 'fibonacci_0.382', 'max_high_look_back_24h', 'min_low_look_back_24h', 'Support']
...
def evaluate_tolerance(model, X_test, y_test, scaler_low, tolerance_percentage=0.2):
    # 预测
    preds = model.predict(X_test)
    
    # 逆缩放预测值
    preds_inverse = scaler_low.inverse_transform(preds.reshape(-1, 1))
    y_test_inverse = scaler_low.inverse_transform(y_test.reshape(-1, 1))

    # 计算容忍范围内的准确性和总损失
    correct = 0
    total_loss = 0
    for pred, true in zip(preds_inverse, y_test_inverse):
        tolerance = true * tolerance_percentage / 100
        if abs(pred - true) <= tolerance:
            correct += 1
        total_loss += (0 if abs(pred - true) <= tolerance else (pred - true) ** 2)
...
col = ['Open', 'High', 'Low', 'Close', 'Current_Max_High', 'Current_Min_Low', 'fibonacci_0.382', 'fibonacci_0.5', 'fibonacci_0.618', 'fibonacci_0', 'fibonacci_1', 'datetime']
df = df[col]
...
# # 进行缩放
# y_low_scaled = scaler_low.fit_transform(y.values.reshape(-1, 1)).flatten()

# X_ochl_scaled_1, X_ochl_y_1, _ = train_data(df, y_low_scaled)

# test_and_validation_size = 0.3
# validation_ratio_of_test = 0.5  # 在测试和验证数据集中，验证集占的比例

# X_train, X_temp, y_train, y_temp = train_test_split(X_ochl_scaled_1, X_ochl_y_1, test_size=test_and_validation_size, shuffle=False)
...
# # 预测最低点价格
# dense_low = Dense(128, activation='relu')(lstm_layer)
# dense_low = Dropout(0.3)(dense_low)
# dense_low = Dense(64, activation='relu')(dense_low)
# dense_low = Dropout(0.3)(dense_low)
# output_low = Dense(1, activation='linear', name='low_price_output')(dense_low)

# # 预测最低点位置
# # concat_layer = Concatenate()([lstm_layer, output_low])
# # dense_pos = Dense(32, activation='relu')(concat_layer)
# # output_pos = Dense(1, activation='linear', name='low_pos_output')(dense_pos)

# # 创建和编译模型
# model = Model(inputs=input_layer, outputs=output_low)

# optimizer = Adam(learning_rate=0.002)
# # model.compile(optimizer=optimizer, loss={'low_price_output': 'mse', 'low_pos_output': 'mse'}, metrics={'low_price_output': 'mae', 'low_pos_output': 'mae'})
# model.compile(optimizer=optimizer, loss='mse', metrics=['mae'])
# tolerance = 0.2
# # model.compile(optimizer=optimizer, loss=lambda y_true, y_pred: tolerance_mse(y_true, y_pred, scaler_low, tolerance=tolerance), metrics=[lambda y_true, y_pred: tolerance_mae(y_true, y_pred, scaler=scaler_low, tolerance=tolerance)])
...
# # 训练模型
# history = model.fit(X_train, y_train, epochs=epochs, batch_size=batch_size, validation_data=(X_val, y_val), callbacks=[reduce_lr, callback])

# # 预测
# # preds = model.predict(X_test)

# # 评估模型
# accuracy, avg_loss = evaluate_tolerance(model, X_test, y_test, scaler_low)
...
#     'Predicted_Low': preds_inverse.flatten(),
#     'Actual_Low': y_test_inverse.flatten()
# }, index=df.index[-len(y_test_inverse):])

# # 合并预测结果到原始 DataFrame
# df['Predicted_Low'] = np.nan
# df.loc[results.index, 'Predicted_Low'] = results['Predicted_Low']

# # 合并实际结果到原始 DataFrame
# df['Actual_Low'] = np.nan
# df.loc[results.index, 'Actual_Low'] = results['Actual_Low']

# # 打印合并后的 DataFrame
# # print(df)

# # 保存合并后的 DataFrame 到 CSV 文件（可选）
# df.to_csv('merged_predictions_vs_actual.csv', index=False)