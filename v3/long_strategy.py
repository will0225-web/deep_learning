...
def backtesting(input_data, lookahead, percentage, prob, volume_threshold, fee=0.0012):
    ...
    for i in range(len(input_data) - lookahead):
        ...
        volume = input_data['Volume'].iloc[i]  # 交易量
        ...
        # 进场多头交易
        if predicted_class == 1 and volume >= volume_threshold:
...
def validation(test_data, model, preprocessor, close_scaler, cols, lookahead, tp, sl, price_related_features, robust_features, volume_threshold, probs_threshold=0.9):
    # 预测并存储结果
    ...
    predicted_classes = (probs >= probs_threshold).astype(int)
    ...
    high_confidence_data = input_data[(probs >= probs_threshold) & (input_data['Volume'] >= volume_threshold)]
    # 计算每个类别的准确率
    ...
    total_profit_percentage, trades = backtesting(input_data, lookahead, tp, probs_threshold, volume_threshold)
...
get_local_file_name = 'ETHUSDT_1m_2023-12-31_23-59-59_2500000_calculated.csv'
# get_local_file_name = ''
...
df.to_csv(f'original_df_{interval}.csv', index=False)

# df = pd.read_csv(f'original_df_{interval}.csv')
with open(f'model_results_long_strategy_grid_{interval}.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    # 写入列名
    writer.writerow(['lookahead', 'tp', 'sl_mu', 'n_estimators', 'max_depth', 'learning_rate', 'volume_threshold', 'probs_threshold', 'total_accuracy', 'class_1_accuracy', 'class_1_denominator', 'class_1_numerator', 'total_profit_percentage'])

    for lookahead in range(60, 180, 30):
        for tp in np.arange(0.01, 0.025, 0.005):
            for sl_mu in [2 ,3]:
                # lookahead = 60
                # tp = 0.01
                sl = tp / sl_mu
                ...
                for n in range(100, 400, 100):
                    for d in range(5, 11, 2):
                        for lr in np.arange(0.1, 0.6, 0.1):
                            for volume_threshold in range(5000, 25000, 5000):
                                for probs_threshold in [0.7, 0.8, 0.9]:
                                    ...
                                    total_accuracy, class_accuracies, class_denominators, class_numerators, total_profit_percentage = validation(test_data, model_big_trend, preprocessor, None, cols, lookahead, tp, sl, None, robust_features, volume_threshold, probs_threshold)

                                    writer.writerow([lookahead, tp, sl_mu, n, d, lr, volume_threshold, probs_threshold, total_accuracy] + class_accuracies + class_denominators + class_numerators + [total_profit_percentage])

                                    print(f"'lookahead': {lookahead}, tp: {tp}, sl_mu: {sl_mu}, n_estimators: {n}, max_depth: {d}, learning_rate: {lr}, 'volume_threshold': {volume_threshold}, 'probs_threshold': {probs_threshold}, total_accuracy: {total_accuracy}, class_1_accuracy: {class_accuracies[1]}, class_1_denominator: {class_denominators[1]}, class_1_numerator: {class_numerators[1]}, total_profit_percentage: {total_profit_percentage}")