...
# # 生成特征和目标
# X, _, _ = signals.prepare_features_and_targets(df, max_lookahead, cols, min_increase=0.003, max_increase=0.015, step=0.001)
...
predict_lookahead = 16
predict_increase = 0.01
model_up.set_params(device='cuda')
model_down.set_params(device='cuda')
for i in range(len(df) - predict_lookahead):
    print(i)
    df = calculate_ahead_high_low(df, predict_increase)
