...
def calculate_ahead_high_low(df):
...
def prepare_features_and_targets(df, max_lookahead):
    ...
    for i in range(len(df) - max_lookahead):
        for lookahead in range(1, max_lookahead + 1):
            high_window = df['High'].iloc[i:i+lookahead].max()
            low_window = df['Low'].iloc[i:i+lookahead].min()
            current_close = df['Close'].iloc[i]
            df = calculate_ahead_high_low(df)
            features.append([
                df['Open'].iloc[i],
                df['High'].iloc[i],
                df['Low'].iloc[i],
                ...
                df['Close_High_pct'].iloc[i],
                df['Close_Low_pct'].iloc[i],
                df['Open_Low_pct'].iloc[i],
                df['High_Low_pct'].iloc[i],
                lookahead
            ])
            targets_up.append(int((high_window - current_close) / current_close >= 0.008))
            targets_down.append(int((current_close - low_window) / current_close >= 0.008))

    return np.array(features), np.array(targets_up), np.array(targets_down)
...
# 生成特征和目标
X, y_up, y_down = prepare_features_and_targets(df, max_lookahead)
...
end_datetime = datetime.datetime.fromtimestamp(end_time_seconds)
end_time_string = end_datetime.strftime("%Y-%m-%d %H:%M:%S")
df = modules_data.get_binance_klines_backward(symbol, interval, end_time_string, 2000, get_local_file_name, is_need_save_original_data=False, is_need_calculated=True)
