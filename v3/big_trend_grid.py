...
#     return df_new
def validation(test_data, model, preprocessor, close_scaler, cols, lookahead, percentage, price_related_features, robust_features):
    # 预测并存储结果
...
# standard_features = []
standard_features = ['Close', 'High', 'Low', 'Open', 'fibonacci_0.382', 'fibonacci_0.5', 'fibonacci_0.618', 'fibonacci_1', 'fibonacci_0', 'EMA_7', 'EMA_25', 'EMA_99', 'Upper Band', 'Lower Band', 'Middle Band', 'VWAP', 'long_tp_price', 'long_sl_price', 'short_tp_price', 'short_sl_price']
minMax_features = []
    ...
    for n in range(100, 1500, 100):
        for d in range(3, 13, 2):
            for lr in np.arange(0.05, 0.55, 0.05):
                ...
                total_accuracy, class_accuracies, class_denominators, class_numerators, total_profit_percentage = validation(test_data, model_big_trend, preprocessor, None, cols, lookahead, percentage, None, robust_features)

                writer.writerow([n, d, lr, total_accuracy] + class_accuracies + class_denominators + class_numerators + [total_profit_percentage])

                print(f"n_estimators: {n}, max_depth: {d}, max_leaves: {0}, learning_rate: {lr}, total_accuracy: {total_accuracy}, class_0_accuracy: {class_accuracies[0]}, class_1_accuracy: {class_accuracies[1]}, class_2_accuracy: {class_accuracies[2]}, class_3_accuracy: {class_accuracies[3]}, class_4_accuracy: {class_accuracies[4]}, class_0_denominator: {class_denominators[0]}, class_1_denominator: {class_denominators[1]}, class_2_denominator: {class_denominators[2]}, class_3_denominator: {class_denominators[3]}, class_4_denominator: {class_denominators[4]}, class_0_numerator: {class_numerators[0]}, class_1_numerator: {class_numerators[1]}, class_2_numerator: {class_numerators[2]}, class_3_numerator: {class_numerators[3]}, class_4_numerator: {class_numerators[4]}, total_profit_percentage: {total_profit_percentage}")